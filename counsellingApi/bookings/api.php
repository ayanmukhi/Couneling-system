...
class apiClass
{
    ...
    public function getBookedCounselor($request, $response, $args) {
        ...
        $rating = $counselorResult->getRecords()[0]->_impl->_fields['Rating_n'][0];
    ...
    //function to update rating of counselor
    function updateRating($request, $response) {

        //creating the object of jwt class
        $jwt = new config\jwt();
       ...
       //parsing the body to get the form value
       $val = json_decode($request->getBody());

       //commands to find the specific record
       $findCommand = $fm->newFindCommand('Bookings_BOOKINGS');
       $findCommand->addFindCriterion('_kp_BookingId_n', '=='.$val->bookingId);
       $result = $findCommand->execute();
       ...
       $rec = $fm->getRecordById('Bookings_BOOKINGS', $rec_ID);
       $rec->setField('Rating_t', $val->rating);
       ...
       if (\FileMaker::isError($result)) {
            $findError = $result->getMessage(). ' (' . $result->code. ')';
            $newresponse = $response->withStatus(404);
            return $newresponse->withJson(['success'=>false, "userRecordUpdationError"=>$findError, "fields" => $val]);

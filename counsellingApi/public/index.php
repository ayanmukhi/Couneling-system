...
//function for saving the file on the file system
...
//function to move the file onto the designated directory
...
//api to get the details of appointed seeker
...
$app->group('/api/v1/bookedCounselor', function() use ($app) {
    ...
    //funtion to updating rating to booked counselor
    $app->put('', function( Request $request, Response $response){
        return bookings\apiClass::updateRating($request, $response);
    });
});
...
//api to get a appoinment record
...
//api to fetch cousnelor details for booking
$app->get('/{id}', function(Request $request, Response $response, array $args) {
